...
public class Tipster extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		//On app load, the curson should be in the Amount field
		...
		/* Attach an OnCheckedChangeListener to the radio group to monitor radio buttons selected by user */
		rdoGroupTips.setOnCheckedChangeListener(new OnCheckedChangeListener() {
			public void onCheckedChanged(RadioGroup group, int checkedId) {
				// Enable/disable Other Tip Percentage field
				if (checkedId == R.id.radioTen || checkedId == R.id.radioFifteen){
					txtTipOther.setEnabled(false);
				...
				if (checkedId == R.id.radioOther) {
					...
					/* Enable the calculate button if Total Amount and No. of People fields have valid values.
					 * Also ensure that user has entered an Other Tip Percentage value before enabling
		...
		/* Attach a KeyListener to the Tip Amount, No. of Peopele and Other Tip
	 ...
	 * Depending on the button clicked, the corresponding method is called.
	 */
	...
	private void calculate() {
		...
		/**
		 * If the user never change his radio selection, then it  means the default selection of 10T is in effect.
		 * But it's safer to verify
		 */
		if (radioCheckedId == -1) {
		...
		/**
		 * If all fields are populated with valid values, then proceed to calculate the tips
		 * 
		 */
		if (!isError) {
	...
	private void reset() {
		...
		rdoGroupTips.check(R.id.radioTen);
		txtTipAmount.requestFocus();
	}
	 ...
	 * 		This is required so that the focus can be set on that field once the dialog is dismmissed.
	 */
